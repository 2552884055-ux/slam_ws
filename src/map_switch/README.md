...
| `tf_robot2map` | `map_switch` / `tf_publish` | 位姿/状态 UDP 上报 |
...
```
scripts/   # 已清空：各层地图节点改由 map_switch 在 C++ 里 rosrun 启动,不再用 .sh
config/
├── floors.yaml      # maps 列表：各楼层 id / pcd / gridmap / 到 map1 的坐标变换
└── mid360.yaml      # tf_publish 用：frame/topic/UDP/频率/开关 等
launch/
└── project.launch   # 唯一保留的 launch：整体工程入口
                     #   加载全局参数(mid360.yaml/floors.yaml + FAST-LIO 参数)、静态 TF、
                     #   map_switch_node、pointcloud_to_laserscan、rviz
                     #   各层地图节点(map_publishe/map_server/global_localization/
                     #   transform_fusion/tf_robot2map/laserMapping)由 map_switch_node 运行时 rosrun 拉起
```
...
- 各被拉起节点所属包（`pcl_ros`、`map_server`、`fast_lio_global`、`map_switch`）已编译/可被 rosrun 找到；
