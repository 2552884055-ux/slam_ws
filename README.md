...
| 程序 | 作用 | 文档 |
|----|------|------|
| **[ElevatorControl](src/ElevatorControl/)** | Modbus 电梯控制 + 两阶段地图切换；**TCP(网络) / RTU(串口) 二合一**(同一套代码,构造时选传输方式)；RTU 支持**串口自动识别 + 运行时 USB 号变化自愈** | [README](src/ElevatorControl/README.md) |
...
└── src/
    ...
    ├── ElevatorControl/        # 上位机:Modbus 电梯控制 + 两阶段地图切换,TCP/RTU 二合一,RTU 串口自动识别/自愈(非 ROS 包)
