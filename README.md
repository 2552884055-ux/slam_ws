...
| 包 | 作用 | 文档 |
|----|------|------|
| **[map_switch](src/map_switch/)** | 多楼层 / 多区域地图切换（TCP 控制）+ 位姿/激光 UDP 上报（`tf_publish`） | [README](src/map_switch/README.md) |
...
`auto_start.sh` 会依次在新终端启动 Livox 驱动与 `map_switch project.launch`。
...
6. **楼层切换坐标 / 脚本路径**：`map_switch/config/floors.yaml` 的各楼层 `start_script` 与 `tx/ty/theta_to_map1`，以及各触发脚本里的绝对路径（默认 `/home/orangepi/slam_ws/...`）。
